...
# # Global variable to track the previous BTC VALUE USD
# prev_btc_value_usd = 0.0
...
#         # Calculate profit/loss (unrealized) compared to previous BTC VALUE USD
#         profit_loss_usd = 0.0
#         if prev_btc_value_usd > 0:
#             profit_loss_usd = round(btc_value_usd - prev_btc_value_usd, 2)
...
#         }
#         await log_trade(trade_record)
#         print(f"[OK] Buy executed: approx {approx_quantity:.6f} BTC at ${current_price:,.2f} ({trade_type}) for ${amount_usd}, Profit/Loss: ${profit_loss_usd}")
#         # Update previous BTC value for the next trade
#         prev_btc_value_usd = btc_value_usd
#         return trade_record
#     except Exception as e:
#         print(f"[ERROR] Buy failed: {e}")
#         return None
...
#         # Calculate profit/loss compared to previous BTC VALUE USD, adjusted for sold value
#         profit_loss_usd = 0.0
#         if prev_btc_value_usd > 0:
#             # Adjust for the value of BTC sold
#             sold_value_usd = quantity * current_price
#             profit_loss_usd = round((btc_value_usd + sold_value_usd) - prev_btc_value_usd, 2)
...
#         }
#         await log_trade(trade_record)
#         print(f"[OK] Sell executed: {quantity_str} BTC at ${current_price:,.2f} ({trade_type}), Profit/Loss: ${profit_loss_usd}")
#         # Update previous BTC value for the next trade
#         prev_btc_value_usd = btc_value_usd
#         return trade_record
#     except Exception as e:
...
#             # Calculate minimum USDT for 0.0001 BTC and MIN_NOTIONAL
#             min_buy_usdt = max(round(0.0001 * current_price, 2), 5)  # Ensure at least 5 USDT for MIN_NOTIONAL
#             print(f"[TEST] Minimum USDT required for buy: ${min_buy_usdt:,.2f}")
